...
function SlotPicker({ pkg, days, selectedDay, onDaySelect, selectedTime, setSelectedTime, onAddToCart, slots, loadingSlots, slotsError }) {
        ...
        {selectedDay && selectedTime && selectedSlot?.available && (
          ...
          >
            ...
            <div className="text-xs text-gray-400">
              {selectedDay.toLocaleDateString('en-IN', { weekday: 'short', day: 'numeric', month: 'short' })} · {fmt12(selectedTime)} · {pkg.duration}
            </div>
          </motion.div>
        )}
...
export default function YogaDetail() {
  ...
  async function handleDaySelect(day) {
    ...
    try {
      const res = await fetch(`/api/availability?date=${dateStr}&serviceSlug=${pkg.slug}`)
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#fbfaf7]">
      <NextSeo
        title={`${pkg.title} — Book Yoga Session`}
        description={`${pkg.excerpt} ₹${pkg.price.toLocaleString('en-IN')} · ${pkg.duration} · ${pkg.mode}. Book your session with MindVeda now.`}
        canonical={pkgUrl}
        openGraph={{
          url: pkgUrl,
          title: `${pkg.title} — MindVeda Yoga`,
      ...
      <main className="flex-1 pb-24 lg:pb-0">
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 pt-4 sm:pt-6">
          <nav className="flex items-center gap-1.5 text-xs sm:text-sm text-gray-400 flex-wrap">
            ...
            <span>/</span>
            <Link href="/yoga" className="hover:text-[#2d4f3a]">Yoga</Link>
            <span>/</span>
          ...
          <div className="flex flex-col lg:grid lg:grid-cols-[1fr_380px] gap-6 lg:gap-8 lg:items-start">
            ...
            <div className="flex flex-col gap-4 sm:gap-5">
              ...
              <div className="bg-gradient-to-r from-[#edf5ee] to-[#f5f0e8] rounded-2xl sm:rounded-3xl p-4 sm:p-6 flex gap-4 items-start">
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-2 flex-wrap mb-1">
                    <span className="text-[10px] sm:text-xs font-semibold text-[#2d4f3a] uppercase tracking-wider">Yoga Session</span>
                    {pkg?.mode && (
                  ...
                  <h1 className="text-xl sm:text-2xl md:text-3xl font-semibold text-[#1a3520] leading-snug">{pkg?.title}</h1>
                  <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-2">
                    ...
                    {pkg?.sessionsLabel && (
                      <span className="text-xs font-semibold text-brand bg-[#edf6ef] px-2 py-0.5 rounded-full">{pkg.sessionsLabel}</span>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div className="card-anim bg-white rounded-2xl p-4 sm:p-6 shadow-sm border border-gray-100">
                <h2 className="text-base sm:text-lg font-semibold text-[#1a3520] mb-2 sm:mb-3">About This Session</h2>
              ...
              <div className="card-anim bg-white rounded-2xl p-4 sm:p-6 shadow-sm border border-gray-100">
                ...
                <ul className="space-y-2 sm:space-y-2.5">
                  {[
                    'Personalized yoga guidance by certified instructor',
                    'Asana, pranayama & mindfulness techniques',
                    'Secure online session with calendar reminders',
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
                {[
                  ['Choose', 'Pick the yoga session for your needs.'],
                  ['Book a Slot', 'Add to cart with your chosen time.'],
                  ['Join Online', 'Checkout and join your session.'],
                ].map(([title, desc], i) => (
                  <div key={title} className="card-anim bg-white rounded-2xl p-4 shadow-sm border border-gray-100 flex sm:flex-col items-start gap-3 sm:gap-0">
              ...
              <div className="flex flex-wrap gap-2">
                {['Online Sessions', 'Certified Instructor', 'Secure Booking', 'Flexible Reschedule'].map(t => (
